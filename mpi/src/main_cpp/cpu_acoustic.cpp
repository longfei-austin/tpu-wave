...
tic = MPI_Wtime();
    ...
    // ---- "prepare" the values for variables from ns_config
    {
        ...
        for ( char dir : {'x','y'} )
        {
            ...
            //       but their paddings are "asymmetric" w.r.t. 'L' and 'R'.
...
tic = MPI_Wtime(); toc = tic;
    ...
    for (int it=0; it<ns_config::Nt; it++)
    {
        ...
        {   
            ...
            if ( ns_mpi::rank==0 ) { printf("\n"); fflush(stdout); }
            if ( ns_mpi::rank==0 ) 
                { double ttoc = MPI_Wtime(); printf("\n time for 1000 time steps : %5.4f\n", ttoc - toc); toc = ttoc; fflush(stdout); }
        }
        

        // update soln_Vx
        {
            ns_action::comm_to_halo<'x'> ( ns_meshes::mesh_P );
        ...
        // update soln_Vy
        {
            ns_action::comm_to_halo<'y'> ( ns_meshes::mesh_P );
        ...
        // update soln_P
        {
            ns_action::comm_to_halo<'x'> ( ns_meshes::mesh_Vx );
            ns_action::comm_to_halo<'y'> ( ns_meshes::mesh_Vy );
...
tic = MPI_Wtime();
    // ---- output the solution and energy
    {
        ...
        // [2024/06/20]
        // NOTE: We need the above barrier. Otherwise, the folder creation may not have been
        //       reflected to the writing rank at the time of writing, which can lead to the
        //       following error message:
        //        x1922c3s4b0n0.hostmgmt2001.cm.americas.sgi.com: rank 1 died from signal 11
        //        x1922c3s4b0n0.hostmgmt2001.cm.americas.sgi.com: rank 0 died from signal 15
        //       See the misc folder "write_MPI_finalize_bug_report" for more information.
...
toc = MPI_Wtime(); if ( ns_mpi::rank==0 ) { printf("\n write output time : %5.4f\n", toc - tic); }
    ...
    MPI_Barrier(MPI_COMM_WORLD);  // test "core dump and no write" finish
