...
namespace namespace_mpi
{
    ...
    { { 'x' , std::map<char , int> { {'L' , 0 } , {'R' , 0 } } } ,
      { 'y' , std::map<char , int> { {'L' , 0 } , {'R' , 0 } } } };
    ...
    inline int ndims = 2;
    inline int dims[2] = {1,1};  // Is this init method in the standard?
    inline std::map<char , int> map_dim { {'x',1} , {'y',1} };
// we can use reference as map value (which would be safer; no need to assign twice)

    inline int periods[2] = {true,true};    // not used in the singlecore version
    inline int reorder = true;              // not used in the singlecore version

    inline int coords[2] = {0,0};// Is this init method in the standard?
    inline std::map<char , int> map_coord { {'x',0} , {'y',0} };
// we can use reference as map value (which would be safer; no need to assign twice)
}
...
public:
    ...
    std::map<char , std::map<char , ns_prcs::precision> > type_send
    { { 'x' , std::map<char , ns_prcs::precision> { {'L' , -1 } , {'R' , -1 } } } ,
      { 'y' , std::map<char , ns_prcs::precision> { {'L' , -1 } , {'R' , -1 } } } };

    std::map<char , std::map<char , ns_prcs::precision> > type_recv
    { { 'x' , std::map<char , ns_prcs::precision> { {'L' , -1 } , {'R' , -1 } } } ,
      { 'y' , std::map<char , ns_prcs::precision> { {'L' , -1 } , {'R' , -1 } } } };
    ...
    void initialize ( char T_x, char T_y )
    {
...

        {
            ...
            // 
            // [2023/12/19] NOTE: Be careful of the duplication on 'N' grid. (The formulas to arrive at
            //                    the above formula are actually very "complicated".) 
...
{
    template<char dir>
    void copy_to_halo ( class_mesh & mesh )
    {
        memory_accessor<ns_prcs::precision,2> & soln = mesh.hst_soln;

        for ( char send_side : {'L','R'} )
        {
            long len_x = mesh.comm_len.at(dir).at(send_side).at(0);
            long len_y = mesh.comm_len.at(dir).at(send_side).at(1);

            long send_bgn_x = mesh.send_bgn.at(dir).at(send_side).at(0);
            long send_bgn_y = mesh.send_bgn.at(dir).at(send_side).at(1);

            char recv_side = ns_h_char::flip_side(send_side);

            long recv_bgn_x = mesh.recv_bgn.at(dir).at(recv_side).at(0);
            long recv_bgn_y = mesh.recv_bgn.at(dir).at(recv_side).at(1);

            for ( long ix=0; ix<len_x; ix++ )
            for ( long iy=0; iy<len_y; iy++ )
            {
                soln.at(recv_bgn_x+ix , recv_bgn_y+iy) = soln.at(send_bgn_x+ix , send_bgn_y+iy);
            }
        }
    }
